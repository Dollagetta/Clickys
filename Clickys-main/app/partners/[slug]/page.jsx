...
import { FiArrowLeft, FiInstagram, FiFacebook, FiMessageCircle } from "react-icons/fi";
...
export default async function PartnerPage({ params }) {
  ...
  const mappedProducts = featured_products.map((p, idx) => ({
    ...
    description: typeof p.product_description === "string" ? p.product_description : p.product_description?.[0]?.text || "",
  }));
        ...
        <div className="bg-white rounded-3xl overflow-hidden shadow-xl border border-gray-100 flex flex-col lg:flex-row h-full group relative">
          <div className="relative lg:w-5/12 xl:w-1/2 h-80 lg:h-auto min-h-[380px] shrink-0 overflow-hidden bg-gray-100">
            ...
            <div className="absolute top-6 left-6 z-30">
              <Link href="/" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-white/90 backdrop-blur-md text-gray-800 hover:bg-white hover:text-black transition-all font-medium">
          ...
          <div className="flex-1 flex flex-col p-8 lg:p-12">
            ...
            {storePromo && (
              <div className="mb-6 inline-block bg-orange-600 text-white text-xs font-bold px-5 py-2 rounded-full shadow-lg uppercase tracking-widest self-start overflow-hidden">
                <span className="animate-promo-slow block px-2">{storePromo}</span>
              </div>
            )}
            <div className="prose text-gray-600 leading-relaxed text-[15.5px] lg:text-base mb-10 flex-1">
              {typeof partner_description === "string" ? partner_description : partner_description?.[0]?.text}
            </div>
            <div className="flex flex-wrap gap-4 mt-auto">
              {whatsapp_number && (
                <a href={`https://api.whatsapp.com/send/?phone=${String(whatsapp_number).replace(/[\s-+]/g, "")}&text&type=phone_number&app_absent=0`} target="_blank" rel="noreferrer" className="flex-1 sm:flex-none inline-flex items-center justify-center px-7 py-4 bg-green-500 hover:bg-green-600 text-white rounded-2xl font-semibold transition-all active:scale-95">
                  <FiMessageCircle className="mr-2 text-xl" /> Contact Seller
                </a>
              )}
              {instagram_link?.url && (
                <a href={instagram_link.url} target="_blank" rel="noreferrer" className="flex-1 sm:flex-none inline-flex items-center justify-center px-7 py-4 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-2xl font-semibold transition-all active:scale-95">
                  <FiInstagram className="mr-2 text-xl" /> Instagram
                </a>
              )}
              {facebook_link?.url && (
                <a href={facebook_link.url} target="_blank" rel="noreferrer" className="flex-1 sm:flex-none inline-flex items-center justify-center px-7 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-semibold transition-all active:scale-95">
            ...
            <div className="mb-10 overflow-hidden rounded-2xl bg-orange-600 text-white py-4 px-6 shadow-lg animate-pulse">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <p className="font-extrabold text-lg md:text-xl italic">{storePromo} — Shop Now!</p>
                </div>
              </div>
            </div>
          )}
          <h2 className="text-3xl font-bold text-gray-900 mb-10 px-2">Products from {partner_name}</h2>
          <PartnerProductsClient products={mappedProducts} themeColor={theme_color} />
        </div>
      </div>
    </div>
  );
